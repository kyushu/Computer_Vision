...
class BOVWIndexer(BaseIndexer):
    ...
    def add(self, hist):
        # hist 是 Sparse-Matrix ( 以 csr_matrix 建立的 )

        # 將新的 hist 加到 bovwBuffer 新的 row
        self.bovwBuffer = BaseIndexer.featureStack(hist, self.bovwBuffer, stackMethod=sparse.vstack)

        # scipy.sparse.csr_matrix.toarray : 是返回 "稀疏矩陣"
        # hist.toarray() 的格式是 [hist 的稀疏矩陣], 所以 hist.toarray()[0] 是 hist 的稀疏矩陣
